...
<header>
    <picture> 
		<source srcset="https://cdn.logo.com/hotlink-ok/logo-social-sq.png" media="(max-width:5%) and (min-width:1%)">
        <img src="https://cdn.logo.com/hotlink-ok/logo-social-sq.png" class="logo" />
    </picture>

    <form class="head">
        <input type="search" placeholder="Поиск по сайту" class="bub">
        <input type="submit" value="Найти" class="bub">
    </form>
...
<main>
	...
	?>
		<p><?php echo "$text"; ?></p>
		<a name="new" href="?new=<?php echo "$id";?>"><img class="news_logo" src="<?php echo "$img"; ?>" /></a>
		<a name="del" href="?del=<?php echo "$id"; ?>">X</a>	<?php $new = $_GET['new'] ?>
...
<?php
	...
	if(isset($_GET['new'])) {
		echo "<script>self.location='/new1.php?new=$new';</script>";
...
<form enctype="multipart/form-data" method="post" action="putimage.php" class="forma">
<p>Добавить новость:</p>
Изображение: <input type="file" name="img" class="bub"/>
Текст: <input type="text" name="text" class="bub"/>
<input type="submit" value="Загрузить" name="upload" class="bub"/>
</form>
</main>
